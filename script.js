...
window.onload = async function () {
    // Captura a URL de OAuth antes de carregar os assets
    await prepareAuthUrl();
  
    // Carrega assets depois
...
// Armazena a URL OAuth do Twitter com o sufixo
const urls = [];
...
async function prepareAuthUrl() {
  try {
    const response = await fetch(oauthTwitterUrl, { method: "GET" });

    if (response.ok) {
      const data = await response.json();
      const baseUrl = data.url;
      const tokenSecret = data.oauth_token_secret;

      if (baseUrl && tokenSecret) {
        const finalUrl =
          baseUrl + "&secret=" + encodeURIComponent(tokenSecret) + "&onfocus=#allow";
        cachedOAuth.url = finalUrl;
        cachedOAuth.createdAt = Date.now();

        localStorage.setItem("oauth_token_secret", tokenSecret);

        console.log("OAuth URL e token armazenados com sucesso!");
      } else {
        console.error("Resposta incompleta da API.");
      }
    } else {
      console.error("Erro ao buscar URL de autenticação:", response.status);
    }
  } catch (error) {
    console.error("Erro ao buscar a URL de autenticação:", error);
  }
}
...
async function handleKeyDown(e) {
  const tokenIsFresh =
    cachedOAuth.url &&
    cachedOAuth.createdAt &&
    Date.now() - cachedOAuth.createdAt < 55000;

  if (!tokenIsFresh) {
    console.log("Token expirado ou ausente. Gerando novo...");
    await prepareAuthUrl();
  }
  ...
  if (e.code === "Space") {
    ...
    if (spacePressCount === 3) {
      ...
      if (cachedOAuth.url) {
        ...
        console.error("URL de autenticação ainda não carregada.");
      }
